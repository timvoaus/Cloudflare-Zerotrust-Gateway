 ...
 * Inspects existing lists starting with "CZGS List"
...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  // Fetch list items with bounded concurrency
  console.log(`Fetching items from ${czgsLists.length} lists with concurrency ${CZGS_API_CONCURRENCY}...`);
  console.log(`CZGS_PROGRESS|phase=fetch|current=0|total=${czgsLists.length}|message=Fetching list items...`);
  const domainsByList = {};
  await runWithConcurrency(
    ...
    },
    {
      ...
      onProgress: (completed, total) => {
        ...
        console.log(`CZGS_PROGRESS|phase=fetch|current=${completed}|total=${total}|message=Fetched ${completed}/${total} lists...`);
      },
    }
  );
  console.log(`CZGS_PROGRESS|phase=fetch|current=${czgsLists.length}|total=${czgsLists.length}|message=Fetch complete - ${czgsLists.length} lists`);
  ...
  console.log(`CZGS_PROGRESS|phase=sync|current=0|total=${totalPatches}|message=Syncing ${totalPatches} list patches...`);
  ...
  for(const [listId, patch] of patchEntries) {
    ...
    console.log(`CZGS_PROGRESS|phase=sync|current=${patchNum}|total=${totalPatches}|message=Patching "${listName}" (${appends} adds, ${removals} removes)`);
 ...
 * Inspects existing lists starting with "CZGS List - Chunk <number>"
...
export const defragmentZeroTrustLists = async () => {
  ...
  // Fetch all the items in the lists with bounded concurrency
  console.log(`Fetching items from ${czgsLists.length} lists with concurrency ${CZGS_API_CONCURRENCY}...`);
  const allEntries = [];
  ...
  const patchEntries = Object.entries(patches);
  const totalPatches = patchEntries.length;
  console.log(`CZGS_PROGRESS|phase=defrag|current=0|total=${totalPatches}|message=Defragmenting ${totalPatches} lists...`);
  ...
  for(const [listId, patch] of patchEntries) {
    ...
    const listName = czgsLists.find(list => list.id === listId)?.name || listId;
    console.log(`Updating list "${listName}"${appends ? `, ${appends} additions` : ''}${removals ? `, ${removals} removals` : ''}`);
    console.log(`CZGS_PROGRESS|phase=defrag|current=${patchNum}|total=${totalPatches}|message=Patching "${listName}" (${appends} adds, ${removals} removes)`);
    await patchExistingListChunked(listId, patch, listName);
  }

  console.log(`CZGS_PROGRESS|phase=defrag|current=${totalPatches}|total=${totalPatches}|message=Defragment complete - ${entriesToMove.length} entries moved`);
  ...
  // This is important to capture any manually created lists starting with "CZGS List"
...
export const upsertZeroTrustDNSRule = async (lists, listName) => {
  ...
  if (managedLists.length === 0) {
    ...
    console.log("Run 'Quick Update' first to create lists and populate the blocklist.");
    return;
  }
